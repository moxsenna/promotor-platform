...
PromotorClass does not own a duplicate `contacts` table on the shared platform.

Canonical Shared Core table:
...
PromotorClass accesses identity through:
...
PromotorClass persists business-relevant learning signals.

It does **not** own canonical `next_actions`.
...
priority INT
reason TEXT
recommended_action_type TEXT nullable
recommended_action_reason TEXT nullable
status TEXT
created_at TIMESTAMP
...
PromotorClass-owned migrations:

```text
class_001 programs
class_002 modules
class_003 lessons
class_004 lesson_resources
class_005 enrollments
class_006 lesson_progress
class_007 reflection_questions
class_008 reflection_responses
class_009 ctas
class_010 learning_events
class_011 learning_signals
class_012 integration_outbox
class_013 indexes
```

Do not create PromotorClass-owned copies of:
...
- learning events
- learning signals and Flow action references
...
Wire PromotorClass to the canonical Shared Core contact layer before enrollment.
...
Use the shared E.164 normalizer from Platform Core. Do not implement a second phone normalizer inside PromotorClass.
...
# 13A. M2A — Shared Platform Contract Lock

## Objective

Prevent PromotorFlow and PromotorClass from creating competing identity or action models.

## Tasks

- wire `INTEGRATION_CONTRACT.md`,
- consume Shared Core Organization/User/Contact,
- use shared E.164 phone normalization,
- define runtime schemas for both adapters,
- create `learning_signals`,
- create `integration_outbox`,
- verify PromotorClass has no canonical `next_actions` table.

## Acceptance Criteria

- [ ] Same contact ID works in both apps.
- [ ] `0812`, `62812`, and `+62812` resolve to one identity.
- [ ] Cross-org integration access is rejected.
- [ ] Flow is the only canonical NextAction owner.
- [ ] Adapter contracts validate at runtime.
...
# 28. M17 — PromotorFlow + Reverse Adapter

## Objective

Implement the clean two-way integration contract.
...
`createNextAction()` creates/reuses the canonical Flow action.

Class does not persist a second canonical NextAction.

## Flow → Class

PromotorClass exposes:
...
Flow completion can therefore:

```text
list aftersales programs
→ human chooses
...
No direct arbitrary cross-table mutation.
...
- [ ] Class works while Flow is unavailable.
- [ ] Learning signal survives integration outage.
- [ ] Request queues/retries.
- [ ] Retry creates one canonical Flow action.
- [ ] Class has no canonical `next_actions` table.
- [ ] Flow reads learning context.
- [ ] Flow lists eligible programs.
- [ ] Flow enrolls same `contact_id`.
